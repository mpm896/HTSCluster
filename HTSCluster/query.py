...
class Query:
    def __init__(self, SMILES: List[str] | DataFrame, fptype: str = "rdkit") -> None:
        self.SMILES: List[str] | DataFrame = (
            DataFrame({"SMILES": SMILES}) if isinstance(SMILES, list) else SMILES
        )
    ...
    def query_neighbors(
        self, df: DataFrame, n_neighbors: int = 50
    ) -> dict[str, DataFrame]:
        """
        ...
        _lib_mols = [
            ...
            for (s) in tqdm(df["SMILES"], desc="Getting mols from SMILES")
        ]
        ...
        for i in tqdm(
            range(len(self.SMILES)), desc="Querying for nearest neighbors..."
        ):
            kneighbors = neigh.kneighbors(np.array(self._fps[i]).reshape(1, -1))
            self.query_neighbors[self.SMILES[i].item()] = DataFrame(
                df[kneighbors[1][0], :]
            )
    ...
    def assign_query_hits(self, df: DataFrame) -> dict[str, DataFrame]:
        ...
        for c in self.query_neighbors.copy():
            df_assigned = DataFrame(self.query_neighbors[c]["SMILES"]).with_columns(
                pl.lit(None, dtype=pl.String).alias("IS_HIT")
            )
