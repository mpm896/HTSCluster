...
def get_fps(fptype: str, mols: List | pl.Series) -> List[Mol]:
    ...
    if fptype == "rdkit":
        return [
            ...
            for x in tqdm(mols, desc="Calculating chemical fingerprints...")
        ]
    # If fptype is 'morgan'
    return [
...
def insert_clusters(clusters: ChemicalCluster, df: DataFrame) -> DataFrame:
    ...
    return df.with_columns(pl.lit(clusters.clusters).alias(f"{cluster_type} Cluster"))
...
def insert_mols(mols: List[np.ndarray], df: DataFrame) -> DataFrame:
    ...
    new_df = df.with_columns(pl.Series(mols, dtype=pl.Object).alias(column_name))
    return new_df
...
# %%
def write_csv(
    df: DataFrame, path: Path | str = Path.cwd(), base_name: str = "cluster_results.csv"
) -> None:
