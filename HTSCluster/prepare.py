...
def check_input(args: argparse.Namespace) -> None:
    ...
    if not args.query:
        if (not args.filename) and not (args.hits or args.lib):
            raise ValueError(
                "Must provide at least one file, either by itself or by specifying the hits and the library with --hits and --lib, respectively"
            )

        if args.filename and (args.hits or args.lib):
            raise ValueError(
                "Provide either one file, or specify both a hits and library file."
            )
    ...
    # If Querying
        ...
        if not args.filename or sum(map(bool, [args.hits, args.lib])) == 0:
            raise ValueError(
                "Must pass in a filename containing the SMILES to search for neighbors. Must also provide a hits or library file (or both) with --hits and --lib, respectively"
            )
...
def file_to_df(file: str) -> DataFrame:
    ...
    if Path(file).suffix == ".csv":
        ...
        return fix_column_names(
            (pl.read_excel(file).select(cs.contains("SMILES")).drop_nulls())
        )
    else:
        raise Exception(
            "Not a supported file type. File types accepted are .csv and .xlsx (Excel)"
        )
...
def prepare_clusters(args: argparse.Namespace) -> dict[str, DataFrameClusters]:
    ...
    df_clusters: dict[str, DataFrameClusters] = {
        "input": {"df": inputdf, "cluster": None},
        "hits": {"df": hits, "cluster": None},
        "lib": {"df": lib, "cluster": None},
    }
...
def prepare_query(
...
) -> tuple[Query, DataFrame, Optional[DataFrame]]:
    """
    ...
    if sum(map(bool, [args.hits, args.lib])) == 1:
        lib = (
            file_to_df(args.lib.name)
            if args.lib
            else file_to_df(args.hits.name)
            if args.hits
            else None
        )
        hits = None
