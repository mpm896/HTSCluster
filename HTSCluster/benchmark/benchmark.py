...
def benchmark_write_xlsx() -> None:
    ...
    start = time.time()
    small_df.write_excel(f'{OUT_DIR}/small-pl.xlsx',)
    ...
    start = time.time()
    PandasTools.SaveXlsxFromFrame(small_df_pandas_with_mols, f'{OUT_DIR}/small-pandas-with-images.xlsx', molCol='Molecule', size=(300,300))
    pd_write_excel_with_images_small = time.time() - start

    start = time.time()
    xlsx_from_polarsdf_deprecated(small_df_with_mols, f'{OUT_DIR}/small-pl-with-images.xlsx', molCol='Molecule', size=(300,300))
    pl_likepandastools_write_excel_with_images_small = time.time() - start

    start = time.time()
    xlsx_from_polarsdf(small_df, f'{OUT_DIR}/small-pl-with-images.xlsx', molCol='Molecule', size=(300,300))
    ...
    print(f'Polars (like Pandas Tools Algo): {pl_likepandastools_write_excel_with_images_small}')
    ...
    start = time.time()
    PandasTools.SaveXlsxFromFrame(large_df_pandas_with_mols, f'{OUT_DIR}/small-pandas-with-images.xlsx', molCol='Molecule', size=(300,300))
    pd_write_excel_with_images_large = time.time() - start

    start = time.time()
    xlsx_from_polarsdf_deprecated(large_df_with_mols, f'{OUT_DIR}/small-pl-with-images.xlsx', molCol='Molecule', size=(300,300))
    pl_likepandastools_write_excel_with_images_large = time.time() - start

    start = time.time()
    xlsx_from_polarsdf(large_df, f'{OUT_DIR}/small-pl-with-images.xlsx', molCol='Molecule', size=(300,300))
    pl_write_excel_with_images_large = time.time() - start

    print('LARGE DATAFRAME: 43,000 COMPOUNDS') 
    print(f'PandasTools: {pd_write_excel_with_images_large}')
    print(f'Polars (like Pandas Tools Algo): {pl_likepandastools_write_excel_with_images_large}')
